...
export const createProject = async (projectData: any) => {
  return await prisma.project.create({
    data: {
      ...projectData,
      tags: {
        connect: projectData.tags.map((tagId: string) => ({ id: tagId })),
      },
      collaborators: {
        create: projectData.collaborators.map((collab: any) => ({
          userId: collab.userId,
          createdAt: collab.createdAt ? new Date(collab.createdAt) : new Date(),
        })),
      },
    },
  });
